...
function searchpage_hook($obj,$row,$rout) {
    ...
    if($rout == 'END')
        searchpage_localsearch_hook(&$obj);
}
...
function searchnews_hook($obj,$row,$rout) {
    ...
    if($rout == 'MIDDLE')
        if(isset($_SESSION['mod_locale']) and $_SESSION['mod_locale'] != 'default') {
            ...
            if(!empty($row['content_news_locale'])) $obj->set('productKey',substr(strip_tags($row['content_news_locale']),0,300)."...");
            if(!empty($row['title_news_locale'])) $obj->set('productName',$row['title_news_locale']);
        }

    if($rout == 'END')
        searchnews_localsearch_hook(&$obj);
}
