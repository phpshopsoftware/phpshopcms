    ...
    function pagemap() {
        ...
        if(is_array($dataArray))
            foreach($dataArray as $row) {
                ...
                if($j==0) {
                    $this->set('pageTitle',$this->PHPShopSystem->getParam('name').' / Ñòğàíèöû');
                    $this->set('pageNumN',"Ğåçóëüòàò: ñòğàíèö - ". count($dataArray));
                }
                else {
    ...
    function newsmap(){
        ...
        if(is_array($dataArray))
            foreach($dataArray as $row) {
                ...
                if($j==0) {
                    $this->set('pageTitle',$this->PHPShopSystem->getParam('name').' / Íîâîñòè');
                    $this->set('pageNumN',"Ğåçóëüòàò: ñòğàíèö - ". count($dataArray));
                }
                else {
                    $this->set('pageTitle',false);
    ...
    function gbookmap(){
        ...
        if(is_array($dataArray))
            foreach($dataArray as $row) {
                ...
                if($j==0) {
                    $this->set('pageTitle',$this->PHPShopSystem->getParam('name').' / Îòçûâû');
                    $this->set('pageNumN',"Ğåçóëüòàò: ñòğàíèö - ". count($dataArray));
                }
                else {
                    $this->set('pageTitle',false);
