...
if(is_array($data))
    foreach($data as $row) {
        ...
        if($PHPShopModules->getParam("base.seourl.seourl_system") != "")
            $stat_pages.= '<loc>https://'.$_SERVER['SERVER_NAME'].'/'.$row['link'].'.html</loc>' . "\n";
        else $stat_pages.= '<loc>https://'.$_SERVER['SERVER_NAME'].'/page/'.$row['link'].'.html</loc>' . "\n";
...
if(is_array($data))
    foreach($data as $row) {
        ...
        // Проверка модуля Seourl
        if($PHPShopModules->getParam("base.seourl.seourl_system") != "")
            $stat_cat.= '<loc>https://'.$_SERVER['SERVER_NAME'].'/cat/'.$row['seoname'].'.html</loc>' . "\n";
        else $stat_cat.= '<loc>https://'.$_SERVER['SERVER_NAME'].'/page/CID_'.intval($row['id']).'.html</loc>' . "\n";
...
if(is_array($data))
    foreach($data as $row) {
        ...
        // Проверка модуля Seourl
        if($PHPShopModules->getParam("base.seourl.seourl_system") != "")
            $stat_news.= '<loc>https://'.$_SERVER['SERVER_NAME'].'/news/ID_'.strip_tags($row['seo_name']).'.html</loc>' . "\n";
        else  $stat_news.= '<loc>https://'.$_SERVER['SERVER_NAME'].'/news/ID_'.intval($row['id']).'.html</loc>' . "\n";
