...
function actionLoad() {
    ...
    if (PHPShopSecurity::true_skin($_POST['template_load'])) {
        ...
        if (strlen($_POST['template_load']) < 20)
            $load = $skin_base_path . '/load/' . $_POST['template_load'] . '/' . $_POST['template_load'] . '_v3.tar.gz';
        else
            $load = null;
        ...
        if (!empty($Content)) {
            $zip = $_SERVER['DOCUMENT_ROOT'] . $GLOBALS['SysValue']['dir']['dir'] . "/UserFiles/Files/" . $_POST['template_load'] . '.tar.gz';
            $zip_load=$_SERVER['SERVER_NAME'].$GLOBALS['SysValue']['dir']['dir'] . "/UserFiles/Files/" . $_POST['template_load'] . '.tar.gz';
            $handle = fopen($zip, "w+");
            ...
            if (is_file($zip)) {
                ...
                @chmod($_classPath . "templates", 0777);

                // Библиотека TAR
                include($_classPath . "lib/tar/pcltar.lib.php");
                $archive = PclTarExtract($zip, $_classPath . "templates/".$_POST['template_load']);
                if (count($archive) != 0) {
                    ...
                    $result = 'Ошибка распаковки файла ' . $_POST['template_load'] . '.tar.gz, нет прав записи в папку phpshop/templates/';
            }
            else
                $result = 'Ошибка записи файла ' . $_POST['template_load'] . '.tar.gz, нет прав записи в папку /UserFiles/Files/';
        }
        else {
            $result = 'Ошибка чтения файла ' . $_POST['template_load'] . '.tar.gz';
        }
    }
